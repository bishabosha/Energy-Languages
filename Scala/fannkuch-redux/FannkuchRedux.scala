...
object FannkuchRedux {
  ...
  private def fannkuch(n: Int): FlipsAndCheck = {
    ...
    def computeBlock(blockIdx: Int): FlipsAndCheck = {
      ...
      def loop(
        ...
        val result @ (newCheckSum, newMaxFlips) = {
          ...
          } else (checkSum, maxFlips)
        }

        if (permutationIdx >= lastPermutationInBlock) {
          result
        } else {
          permute()
    ...
    } yield Future { computeBlock(blockIdx) }

    Await
      .result(Future.sequence(tasks), Duration.Inf)
      .reduce[FlipsAndCheck] {
